...
import mapIndexedDispatch from "src/util/map-indexed-dispatch";
import mapIndexedCommand from "src/util/map-indexed-command";
import mapIndexedSubscriptions from "src/util/map-indexed-subscriptions";
...
export const subscriptions = ({ shared, state, lib }) => {
  const { gifs } = state;
  const mappedSubs = flatMap(gifs, (gif, index) => mapIndexedSubscriptions("updateGif", index, GifStatic.subscriptions({ shared, lib, state: gif })));
  const newGifSub = lib.websocket(`newGif:${state.bucketId.value}`, "receiveNewGif");
